...
// =========================================
// Flat module: V1 normal-input capture adapter.
...
function emitResult(text) {
  ...
  window.dispatchEvent(new CustomEvent("haiva:v1-capture-result", { detail: { text: normalized, source: "v1" } }));
}

function emitCaptureError(error) {
  if (typeof window === "undefined") return;
  window.dispatchEvent(new CustomEvent("haiva:v1-capture-error", { detail: { source: "v1", error } }));
}
...
function startCapture() {
  ...
  handoffToV1(() => {
    ...
    if (window.HaivaBridge?.startVoiceCapture) {
      try { window.HaivaBridge.startVoiceCapture(); return; } catch (error) {
