...
import { executePhoneCommand } from "./phone-capabilities.js";
...
function getBasicResponse(command) {
  ...
  if (/^(ano ang kaya mo|anong kaya mo|what can you do|what do you do)$/i.test(text)) {
    return "I can chat with you, listen through voice mode, respond by voice, control supported phone capabilities, and connect to my AI brains and tools.";
  }
...
export class HAIVAAssistant {
  constructor() { this.processing = false; }
  ...
  async respond(command) {
    ...
    try {
      ...
      try {
        const phoneResponse = executePhoneCommand(text);
        if (phoneResponse) return phoneResponse;
      } catch (error) {
        console.warn("[HAIVA] Phone capability failed; continuing to AI routing.", error);
      }
