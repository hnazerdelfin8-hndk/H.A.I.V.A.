...
test("boot loader hands off immediately and does not own a second timeout", async () => {
  ...
  assert.doesNotMatch(source, /DOMContentLoaded.*handoffToBoot/s);
});
...
test("runtime probes do not compete with the boot controller", async () => {
  ...
  assert.doesNotMatch(probe, /window\.addEventListener\(["']error["']/);
  assert.doesNotMatch(probe, /window\.addEventListener\(["']unhandledrejection["']/);
  assert.doesNotMatch(controls, /window\.addEventListener\(["']error["']/);
  assert.doesNotMatch(controls, /window\.addEventListener\(["']unhandledrejection["']/);
});
